...
class MagneticSusceptibilityDataset {
	...
	offerAnalysis(analysisJSON) {
		...
		if(this.analysisData.methodId == 33) {
			console.log("MagneticSusceptibilityDataset claiming", this.analysisData);
			claimed = true;
			this.fetchDataset();
		}
	...
	*
	* TODO: We need to aggregate the abundance counting result from different samples. Because it's not really interesting to have abundance divided per sample, I think... CORRECTION: YES IT IS, lIKE, EXTREMELY IMPORTANT
	*
	* Parameters:
	* datasetId
	 */
