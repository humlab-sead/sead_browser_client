...
class MagneticSusceptibilityDataset extends DatasetModule {
	...
	claimDatasets(siteDatasets) {
		let methodDatasets = [];
		for(let key in siteDatasets) {
			let dataset = siteDatasets[key];
            if(this.methodIds.includes(dataset.method_id)) {
                methodDatasets.push(dataset);
                siteDatasets.splice(key, 1);
            }
        }
	...
	makeSection(site, sections) {
		...
		let methodDatasets = this.claimDatasets(site.datasets);
